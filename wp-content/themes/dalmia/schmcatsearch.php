<?php /*Template Name: Scheme Performance Search Result*/ ?>
...
<?php get_header(); ?>
	...
	?>
        ...
        <div class="srchcat">
        <?php
		$path= site_url()."/scheme-performance-search-result";
		echo "<form name='' action='$path' method='post'>";
		$sqlmain = "SELECT DISTINCT scheme_cat FROM $table WHERE scheme_performance_id >1";
		...
		{
		echo '<option value="'. $resultmains->scheme_cat.'">'. $resultmains->scheme_cat.'</option>';
		}
		echo '</select>';
		echo '</form>';
		?>
		...
		?>
            ...
            <?php
			$sql = "SELECT scheme_title,as_on_date FROM $table WHERE scheme_performance_id=2";
			$results = $wpdb->get_results($sql);
			foreach($results as $result)
			{
            echo "<h1>".$result->scheme_title."</h1>";
        ...
        <div style="clear:both"></div>
            ...
            <?php
			$sql = "SELECT *FROM $table WHERE scheme_performance_id>1 AND scheme_cat='$srch'";
			$results = $wpdb->get_results($sql);
            $i=1;
            foreach($results as $result)
			{
            ...
            foreach($results as $result)
			{
			echo "<div class='onediv2'>".$result->scheme_cat."</div>";
			echo "<div style='clear:both'></div>";
			$i++;
			}
			?>
            ...
            foreach($results as $result)
			{
			echo "<div class='onediv2'>".$result->scheme_name."</div>";
			echo "<div style='clear:both'></div>";
			$i++;
			}
			?>
